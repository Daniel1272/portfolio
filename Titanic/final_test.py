...
# Preprocessing: fill missing values, encode features as in training
...
# Prepare meta-features from base models predictions on test set
meta_features = np.column_stack([np.sign(np.array([(alg.predict(X)*alpha) for alg, alpha in zip(algs, algs_alpha)]).sum(axis=0)),
                                ...
                                KNN_model.predict(X_scaled), gaus_model.predict(X),
                                np.where(xgb_clf.predict(X) == 0, -1, 1)])

# Predict final labels with trained meta-model
